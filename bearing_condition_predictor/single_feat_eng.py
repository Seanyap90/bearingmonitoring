...
def ToFrequency(df):
    ...
    Hshort = signal.decimate(df["Hacc"], 2)
    Vshort = signal.decimate(df["Vacc"], 2)
    Hf = np.abs(rfft(Hshort))
    Vf = np.abs(rfft(Vshort))
    x = np.append(Hf, Vf)
    X = np.append(X, np.array([x]), axis=0)

    df_freq = pd.DataFrame(X, columns=headers)
...
def ToTime(df):
    ...
    for i in range(0, len(df), 2560):
        Horizontal = signal.decimate(df["Hacc"], 2)
        Vertical = signal.decimate(df["Vacc"], 2)
